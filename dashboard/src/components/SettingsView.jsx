...
import { ArrowLeft, Database, ArchiveRestore, ChevronRight, Save, Trash2, RefreshCcw, Tag as TagIcon, Pencil, CalendarDays, Book } from 'lucide-react'
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-4">
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-5">
                <div className="text-xs text-slate-500 bg-blue-50 p-3 rounded-xl border border-blue-100 leading-relaxed">
                  绑定存放 Obsidian 笔记的 GitHub 仓库。我们将使用系统全局的 GitHub Token 进行访问，支持解析私有仓库。
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">包含这些文字的文件或文件夹将不会显示在阅读器中。</p>
                </div>
                <button onClick={handleSaveObsidian} className="mt-6 w-full bg-slate-800 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform"><Save size={18} /> 保存配置</button>
              </div>
            </div>
          </motion.div>
        )}
